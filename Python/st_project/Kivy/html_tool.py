...
# 0 TODO Put New Lines at closing tag
# 1 TODO Automate Aria-labelledby or suggestion for id
# 2 TODO implement Previous Button

import kivy
from Kivy.html_Format import HtmlFormat
from Kivy.html_Fix import HtmlTool
from Kivy.link_Text import LinkText
...
class AccessibilityToolApp(App):
    ...
    def __init__(self, **kwargs):
        # Test Area TODO
        self.html_Fix = HtmlTool()
        self.html_Format = HtmlFormat()
        self.link_Text = LinkText()
        ...
        self.html_Format.structure_file(self.file_name)
        self.html_Format.remove_blank_lines(self.file_name)
        ...
        self.label_mid = Label(text="", font_size=10)
        ...
        self.button_unset_03 = Button(text="Add Aria-Labelled by ID's",
                                      background_color=self.color_active)
        ...
        if self.link_Click:
            if self.checkAttribute is False and self.checkAttribute2 is True:
                if self.input_top.focus and keycode == 40:
                    self.matchedLine[self.linkIndex] = self.link_Text.fix(str(self.matchedLine[self.linkIndex]),
                    ...
                    if self.linkIndex == len(self.matchedLine):
                        ...
                        self.link_Text.update_file(self.file_name, self.matchedLine, self.matchedLineNumber)
                        self.link_Click = False
            ...
            if (self.html_Start or self.html_Next) and self.input_top.focus and keycode == 40:
                self.html_Start = False
                self.matchedLine[self.htmlIndex] = self.html_Fix.fix(str(self.matchedLine[self.htmlIndex]),
                                                                     str(self.input_top.text))
                ...
                if self.htmlIndex == len(self.matchedLine):
                    ...
                    self.checkAttribute = False
                    self.label_top.text = "Finished"
                    self.html_Fix.update_file(self.file_name, self.matchedLine, self.matchedLineNumber)
                    self.html_Click = False
                    self.html_Next = False
                    self.button_html_format.background_color = self.color_default
                    self.html_Format.remove_blank_lines(self.file_name)
    ...
    def button_click_reset(self, btn3):
        ...
        del self.matches[:]
        del self.lines[:]
        del self.matchedLine[:]
        del self.matchedLineNumber[:]
        self.linkFixSelected = ''
        self.button_clearHold = ''
        self.button_unset_04Hold = ''
        self.button_unset_05Hold = ''
        ...
        self.label_top.text = ''
        self.label_mid.text = ''
        self.label_tools.text = 'Tools'
        self.label_options.text = 'Options'
        # Start TextInput
        self.input_top.text = ''
        # Start Top Buttons
        self.button_prev.text = 'Previous'
        self.button_unset_01.text = ''
        self.button_next.text = 'Next'
        # Start Middle Buttons
        self.button_start.text = 'Start'
        self.button_unset_02.text = ''
        self.button_reset.text = 'Reset'
        self.button_html_format.text = 'HTML is used to format content'
        self.button_link.text = 'Link text used for multiple different destinations'
        self.button_unset_03.text = ''
        self.button_clear.text = 'Clear After Each Step - On'
        self.button_unset_04.text = ''
        self.button_unset_05.text = ''
    ...
    def button_click_html_format(self, btn4):
        if self.html_Click is False and self.link_Click is False:  # Boolean
            ...
            self.fixes_made = self.html_Format.repair_file(self.file_name)
            del self.lines[:]
            del self.matches[:]
            self.lines = self.html_Fix.get_lines(self.file_name)
            self.matches = self.html_Fix.search_file(self.file_name)
    ...
    def button_click_link_text(self, btn5):
        if self.html_Active is False and self.link_Click is False:
            ...
            del self.lines[:]
            self.lines = self.link_Text.get_lines(self.file_name)
            del self.matches[:]
            self.matches = self.link_Text.search_file(self.file_name)
            if len(self.matches) < 1:
    ...
    def button_click_fill03(self, btn6):
        # TODO Build Aria-Labelled By ID Generator
        if self.html_Active is False and self.link_Click is False:
            pass
        else:
            pass
